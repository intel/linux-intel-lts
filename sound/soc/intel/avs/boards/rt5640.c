			       ...
			       struct snd_pcm_hw_params *params)
{
	...
	/* Format 24/32 for HDAudio is MSB-aligned whereas for SSP its LSB-aligned. */
...
static int avs_create_dai_link(struct device *dev, const char *platform_name, int ssp_port,
			       int tdm_slot, struct snd_soc_acpi_mach *mach,
...
static int avs_rt5640_probe(struct platform_device *pdev)
{
	...
	ret = avs_create_dai_link(dev, pname, ssp_port, tdm_slot, mach, &dai_link);
	if (ret) {
