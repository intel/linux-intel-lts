...
int rt274_mic_detect(struct snd_soc_codec *codec, struct snd_soc_jack *jack)
{
	struct rt274_priv *rt274 = snd_soc_codec_get_drvdata(codec);
...
EXPORT_SYMBOL_GPL(rt274_mic_detect);
...
static const struct snd_kcontrol_new rt274_snd_controls[] = {
	SOC_DOUBLE_R_TLV("DAC0 Playback Volume", RT274_DAC0L_GAIN,
			 RT274_DAC0R_GAIN, 0, 0x57, 0, out_vol_tlv),
	SOC_DOUBLE_R_TLV("DAC1 Playback Volume", RT274_DAC1L_GAIN,
			 RT274_DAC1R_GAIN, 0, 0x57, 0, out_vol_tlv),
	SOC_DOUBLE_R_TLV("ADC0 Capture Volume", RT274_ADCL_GAIN,
			    RT274_ADCR_GAIN, 0, 0x3f, 0, in_vol_tlv),
...
static ssize_t rt274_codec_adb_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt274_priv *rt274 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt274->codec;
	unsigned int val;
	int cnt = 0, i;

	for (i = 0; i < rt274->adb_reg_num; i++) {
		if (cnt + 23 >= PAGE_SIZE)
			break;
		val = snd_soc_read(codec, rt274->adb_reg_addr[i]);
		cnt += snprintf(buf + cnt, 23, "%04x: %04x\n",
			rt274->adb_reg_addr[i], val);
	}

	return cnt;
}

static ssize_t rt274_codec_adb_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == 'R' || buf[0] == 'r') {
		...
		for (i = 0; i < rt274->adb_reg_num; i++) {
			if (rt274->adb_reg_addr[i] < 0xffffffff)
				snd_soc_write(codec, rt274->adb_reg_addr[i],
					rt274->adb_reg_value[i]);
		}
...
static DEVICE_ATTR(codec_reg_adb, 0664, rt274_codec_adb_show,
			rt274_codec_adb_store);
