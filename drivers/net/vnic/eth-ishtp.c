 ...
 * (ISH) using ISH Transport protocol (ISHTP)
 *
...
static const struct ishtp_device_id eth_ishtp_id_table[] = {
	{ .guid = GUID_INIT(0xeb83e1fb, 0x4c61, 0x4829,
		  0x98, 0x4c, 0x3, 0x23, 0xab, 0x4b, 0x41, 0x65) },
	{ }
};
MODULE_DEVICE_TABLE(ishtp, eth_ishtp_id_table);
...
static int eth_ishtp_cl_write(struct eth_ishtp_prv *prv, int *written)
{
	...
	ret = ishtp_cl_send(prv->eth_ishtp_cl, prv->tx_buff, prv->tx_len);
	if (ret)
		goto err_exit;
...
static void eth_ishtp_tx_timeout(struct net_device *dev, unsigned int tx_queue)
{
	...
	dev_warn(prv->dev, "Transmit timeout at %ld, latency %ld\n",
...
static int eth_ishtp_dev_init(struct net_device *dev)
{
	...
	fw_client = ishtp_fw_cl_get_client(ishtp_dev, &eth_ishtp_id_table[0].guid);
	if (!fw_client) {
...
static int eth_ishtp_cl_reset(struct ishtp_cl_device *cl_device)
{
	...
	if (!prv) {
		dev_err(ishtp_device(cl_device),
...
MODULE_AUTHOR("Singaravelan Nallasellan <singaravelan.nallasellan@intel.com>");
