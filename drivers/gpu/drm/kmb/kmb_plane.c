...
static unsigned int check_pixel_format(struct drm_plane *plane, u32 format)
{
	...
	if (init_disp_cfg.format && init_disp_cfg.format != format) {
		DRM_DEBUG("Cannot change format after initial plane configuration");
		return -EINVAL;
	}
				  ...
				  struct drm_plane_state *state)
{
	...
	if (state->crtc_w > KMB_FB_MAX_WIDTH ||
	    state->crtc_h > KMB_FB_MAX_HEIGHT ||
	    state->crtc_w < KMB_FB_MIN_WIDTH ||
	    state->crtc_h < KMB_FB_MIN_HEIGHT)
		return -EINVAL;
	...
	if ((init_disp_cfg.width && init_disp_cfg.height) &&
		...
		DRM_DEBUG("Cannot change plane height or width after initial configuration");
		return -EINVAL;
	}
				    ...
				    struct drm_plane_state *state)
{
	...
	if (num_planes > 1) {
		cb_stride = (md != NULL) ? fb->pitches[1] :
			width*fb->format->cpp[0];
		kmb_write_lcd(kmb,
		...
		if (num_planes == 3) {
			cr_stride = (md != NULL) ? fb->pitches[2] :
				width*fb->format->cpp[0];
			kmb_write_lcd(kmb,
