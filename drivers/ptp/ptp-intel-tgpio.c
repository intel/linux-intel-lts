...
static int intel_tgpio_probe(struct pci_dev *pci, const struct pci_device_id *id)
{
	...
	tgpio->base = pcim_iomap_table(pci)[0];
	if (!tgpio->base) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	pci_set_drvdata(pci, tgpio);

	tgpio->clock = ptp_clock_register(&tgpio->info, &pci->dev);
	...
	irq = pci_irq_vector(pci, 1);
	ret = devm_request_threaded_irq(dev, irq, intel_tgpio_irq,
...
static void intel_tgpio_remove(struct pci_dev *pci)
{
...
MODULE_DEVICE_TABLE(pci, intel_tgpio_id_table);

static struct pci_driver intel_tgpio_driver = {
	.name		= "intel-tgpio",
...
MODULE_DESCRIPTION("Intel Timed GPIO Controller Driver");
