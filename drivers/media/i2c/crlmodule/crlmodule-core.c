			       ...
			       struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		fmt->format = *v4l2_subdev_get_try_format(subdev, state,
				fmt->pad);
		return 0;
	}
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	rval = __crlmodule_get_format(subdev, state, fmt);
				    ...
				    struct v4l2_rect **comps, int which)
{
	...
	if (which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		...
		if (comps) {
			*comps = v4l2_subdev_get_try_compose(subdev, state,
							     CRL_PAD_SINK);
			BUG_ON(!*comps);
		}
	}
				...
				struct v4l2_subdev_selection *sel)
{
	...
	crlmodule_get_crop_compose(subdev, state, crops, &comp, sel->which);

	if (sel->which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		sink_fmt = ssd->sink_fmt;
	} else {
		struct v4l2_mbus_framefmt *fmt =
			v4l2_subdev_get_try_format(subdev, state, ssd->sink_pad);
		sink_fmt.left = 0;
		sink_fmt.top = 0;
			     ...
			     int target)
{
	...
	crlmodule_get_crop_compose(subdev, state, crops, &comp, which);
			      ...
			      struct v4l2_subdev_selection *sel)
{
	...
	*comp = sel->r;
	...
	crlmodule_propagate(subdev, state, sel->which,
			      ...
			      struct v4l2_subdev_selection *sel)
{
	...
	crlmodule_get_crop_compose(subdev, state, crops, NULL, sel->which);
	...
	if (sel->which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		...
		if (sel->pad == ssd->sink_pad) {
			...
			src_size =
				v4l2_subdev_get_try_compose(subdev, state,
							    ssd->sink_pad);
		}
	}
				...
				struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == ssd->source_pad) {
		...
		int rval = __crlmodule_get_format(subdev, state, fmt);
	...
	crlmodule_get_crop_compose(subdev, state, crops, NULL, fmt->which);
	...
	crlmodule_propagate(subdev, state, fmt->which,
			 V4L2_SEL_TGT_CROP);
				...
				struct v4l2_subdev_selection *sel)
{
	...
	switch (sel->target) {
	case V4L2_SEL_TGT_CROP:
		ret = crlmodule_set_crop(subdev, state, sel);
		break;
	case V4L2_SEL_TGT_COMPOSE:
		ret = crlmodule_set_compose(subdev, state, sel);
		break;
	default:
		ret = -EINVAL;
	}
