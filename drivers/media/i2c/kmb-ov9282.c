...
/*
 * kmb-ov9282.c - KeemBay Camera ov9282 Sensor Driver.
 *
...
static const struct kmb_ov9282_reg kmb_ov9282_main[] = {
	{0x3006, 0x02},
	{0x3666, 0x00},
	{0x3823, 0x00},
};

static const struct kmb_ov9282_reg kmb_ov9282_aux[] = {
	{0x3006, 0x00},
	{0x3666, 0x00},
	{0x3823, 0x30},
};
...
{
	u32 lpfr = (mode->lpfr * mode->fps.def) / kmb_ov9282->fps;
				     ...
				     struct v4l2_subdev_format *fmt)
{
	...
	kmb_ov9282->fps = kmb_ov9282->fps ? : mode->fps.def;
	if (kmb_ov9282->fps > mode->fps.max)
		kmb_ov9282->fps = mode->fps.max;
	else if (kmb_ov9282->fps < mode->fps.min)
		kmb_ov9282->fps = mode->fps.min;

	kmb_ov9282_update_fps(kmb_ov9282, mode);
...
static int kmb_ov9282_start_streaming(struct kmb_ov9282 *kmb_ov9282)
{
	...
	case KMB_SYNC_MAIN:
		...
		if (ret) {
			dev_err(kmb_ov9282->dev,
				"fail to write main sensor registers");
			return ret;
		}
		break;
	case KMB_SYNC_AUX:
		...
		if (ret) {
			dev_err(kmb_ov9282->dev,
				"fail to write auxiliary sensor registers");
			return ret;
		}
		break;
	default:
				...
				struct v4l2_subdev_frame_interval *interval)
{
	...
	kmb_ov9282_update_fps(kmb_ov9282, kmb_ov9282->cur_mode);

	kmb_ov9282_update_controls(kmb_ov9282, kmb_ov9282->cur_mode);
...
/**
 * kmb_ov9282_s_power - Set power core operation. Actual power is enabled on
 *                      set stream, this callback is used only for reset formats
 *                      and fps to default values.
 * @sd: pointer to kmb_ov9282 sub-device.
 * @on: power on/off flag.
 *
 * Return: 0 if successful
 */
static int kmb_ov9282_s_power(struct v4l2_subdev *sd, int on)
{
	struct kmb_ov9282 *kmb_ov9282 = to_kmb_ov9282(sd);
...
/* V4l2 subdevice ops */
static struct v4l2_subdev_core_ops kmb_ov9282_subdev_core_ops = {
	.s_power = kmb_ov9282_s_power,
};
...
static const struct v4l2_subdev_ops kmb_ov9282_subdev_ops = {
	.core = &kmb_ov9282_subdev_core_ops,
...
static int kmb_ov9282_get_i2c_client(struct kmb_ov9282 *kmb_ov9282)
{
	...
	kmb_ov9282->client = i2c_new_scanned_device(i2c_adp, &info, addr_list,
						    NULL);
	i2c_put_adapter(i2c_adp);

	return PTR_ERR_OR_ZERO(kmb_ov9282->client);
}
...
MODULE_DESCRIPTION("KeemBay OV9282 Sensor driver");
