			 ...
			 unsigned int stream)
{
	...
	if (which == V4L2_SUBDEV_FORMAT_TRY)
		return v4l2_subdev_get_try_format(subdev, state, pad);
	else
				...
				struct v4l2_subdev_format *fmt)
{
	...
	mutex_lock(&va->mutex);
	fmt->format = *__ti964_get_ffmt(subdev, state, fmt->pad,
				...
				struct v4l2_subdev_format *fmt)
{
	...
	mutex_lock(&va->mutex);
	ffmt = __ti964_get_ffmt(subdev, state, fmt->pad, fmt->which,
				      fmt->stream);
...
static int ti964_rx_port_config(struct ti964 *va, int sink, int rx_port)
{
	...
	case 8:
	case 16:
		port_state2_val = TI964_RAW10_8BIT & (~high_fv_flags);
		vc_mode_reg_index = TI964_RAW10_ID;
		break;
	case 12:
		port_state2_val = TI964_RAW12;
		vc_mode_reg_index = TI964_RAW12_ID;
		break;
	default:
		port_state2_val = TI964_RAW10_NORMAL & (~high_fv_flags);
		vc_mode_reg_index = TI964_RAW10_ID;
		break;
	}
	...
	rval = regmap_write(va->regmap8, TI964_PORT_CONFIG2, port_state2_val);
	if (rval) {
...
static int ti964_register_subdev(struct ti964 *va)
{
	...
	for (i = 0; i < ARRAY_SIZE(ti964_controls); i++) {
		const struct v4l2_ctrl_config *state =
			&ti964_controls[i];
		struct v4l2_ctrl *ctrl;

		ctrl = v4l2_ctrl_new_custom(&va->ctrl_handler, state, NULL);
		if (!ctrl) {
			dev_err(va->sd.dev,
...
static void ti964_gpio_set(struct gpio_chip *chip, unsigned int gpio, int value)
{
