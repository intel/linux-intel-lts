...
/*
 * Keem Bay Camera imx335 Sensor Driver.
 *
...
#define KMB_IMX335_EXPOSURE_S_DEFAULT	0x04
...
static const struct kmb_imx335_reg mode_2616x1964_regs[] = {
	{0x3000, 0x01},
	{0x3002, 0x00},
	{0x300C, 0x3B},
	{0x300D, 0x2A},
...
static const struct kmb_imx335_reg mode_hdr_2dol_15fps_2616x1964_regs[] = {
	{0x3000, 0x01},
	{0x3002, 0x00},
	{0x300C, 0x3B},
	{0x300D, 0x2A},
...
static const struct kmb_imx335_mode supported_modes[] = {
	{
		...
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_2616x1964_regs),
			.regs = mode_2616x1964_regs,
		},
	},
};
...
static const struct kmb_imx335_mode hdr_2dol_supported_modes[] = {
	{
		...
		.reg_list = {
			.num_of_regs =
				ARRAY_SIZE(mode_hdr_2dol_15fps_2616x1964_regs),
			.regs = mode_hdr_2dol_15fps_2616x1964_regs,
		},
	},
};
...
{
	u32 lpfr = (kmb_imx335->cur_mode->lpfr[0] *
		    kmb_imx335->cur_mode->fps.def) / kmb_imx335->fps;

	if (lpfr > KMB_IMX335_LPFR_MAX)
		lpfr = KMB_IMX335_LPFR_MAX;

	if (lpfr < kmb_imx335->cur_mode->height + 96)
		lpfr = kmb_imx335->cur_mode->height + 96;
...
{
	const struct kmb_imx335_mode *mode = supported_modes;
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	kmb_imx335->fps = mode->fps.def;
	kmb_imx335->lpfr = mode->lpfr[0];

	kmb_imx335_update_fps(kmb_imx335);
				...
				struct v4l2_subdev_frame_interval *interval)
{
	...
	fps = (u32)(interval->interval.denominator /
		interval->interval.numerator);
	if (fps < kmb_imx335->cur_mode->fps.min) {
		interval->interval.numerator = 1;
		interval->interval.denominator = kmb_imx335->cur_mode->fps.min;
	} else if (fps > kmb_imx335->cur_mode->fps.max) {
		interval->interval.numerator = 1;
		interval->interval.denominator = kmb_imx335->cur_mode->fps.max;
	}

	kmb_imx335->fps = (u32) (interval->interval.denominator /
				     interval->interval.numerator);
...
static int kmb_imx335_init_controls(struct kmb_imx335 *kmb_imx335)
{
	...
	v4l2_ctrl_cluster(4, &kmb_imx335->exp_ctrl);
...
MODULE_DESCRIPTION("Keem Bay imx335 Sensor driver");
