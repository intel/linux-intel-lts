...
/**
 * mei_virtio_hbuf_depth() - returns depth of FE write buffer.
 * @dev: the device structure
 *
			       ...
			       struct virtio_device *vdev)
{
	...
	struct virtqueue_info vqs_info[] = {
		{ "in", mei_virtio_data_in },
		{ "out", mei_virtio_data_out },
	};
	int ret;
...
static int mei_virtio_probe(struct virtio_device *vdev)
{
	...
	mei_device_init(&hw->mdev, &vdev->dev, false, &mei_virtio_ops);
