...
TRACE_EVENT(drm_file_open,
	...
	TP_fast_assign(
	__entry->drmdev = drmdev;
	__entry->file = file;
	),

	TP_printk("drmdev = %p, file = %p",
		  __entry->drmdev, __entry->file)
);
...
TRACE_EVENT(client_add,
		TP_PROTO(void *file_attr, int sliceidx, int clientid, int codec, int profile, unsigned long width, unsigned long height),

	TP_ARGS(file_attr, sliceidx, clientid, codec, profile, width, height),

	TP_STRUCT__entry(
	__field(void *, file_attr)
	__field(int, sliceidx)
	...
	TP_fast_assign(
	__entry->file_attr = file_attr;
	__entry->sliceidx = sliceidx;
	...
	TP_printk("file_attr %p, sliceidx = %d, clientid = %x, type = %d, profile = %d, width = %ld, height = %ld",
		  __entry->file_attr, __entry->sliceidx,  __entry->clientid, __entry->codec, __entry->profile, __entry->width, __entry->height)
);
...
TRACE_EVENT(client_remove,
	...
	TP_printk("file_attr %p, clientid = %x",
		  __entry->file_attr, __entry->clientid)
);


TRACE_EVENT(cma_alloc,
		TP_PROTO(int sliceidx, int region, void *paddr, int handle, int size),

	TP_ARGS(sliceidx, region, paddr, handle, size),
	...
	TP_printk("sliceidx = %d, region = %d, paddr = %p, handle = %-3d, size = %d",
		  __entry->sliceidx, __entry->region, __entry->paddr, __entry->handle,	__entry->size)
);

TRACE_EVENT(cma_free,
		TP_PROTO(int sliceidx, void *paddr, int handle),

	TP_ARGS(sliceidx, paddr, handle),
	...
	TP_printk("sliceidx = %d, paddr = %p, handle = %-3d,",
		  __entry->sliceidx, __entry->paddr, __entry->handle)
);

TRACE_EVENT(dec_reserve,
		TP_PROTO(int sliceidx, int core, int waittime),

	TP_ARGS(sliceidx, core, waittime),
	...
	TP_printk("sliceidx = %d, core = %d, waittime = %d us",
		  __entry->sliceidx, __entry->core, __entry->waittime)
);

TRACE_EVENT(dec_release,
		TP_PROTO(int sliceidx, int core),

	TP_ARGS(sliceidx, core),
	...
	TP_printk("sliceidx = %d, core = %d",
		  __entry->sliceidx, __entry->core)
);
...
TRACE_EVENT(cache_reserve,
		TP_PROTO(int sliceidx, int waittime),

	TP_ARGS(sliceidx, waittime),
	...
	TP_printk("sliceidx = %d, waittime = %d us",
		  __entry->sliceidx, __entry->waittime)
);

TRACE_EVENT(cache_release,
		TP_PROTO(int sliceidx),
	...
	TP_printk("sliceidx = %d",
		  __entry->sliceidx)
);

TRACE_EVENT(fence_acquirebuf,
	TP_PROTO(void *obj, int handle, int fence_handle, int waittime,  int ret),
	...
	TP_printk("obj = %p, handle = %-3d, fence = %-3d, waittime = %-4d us, ret = %d",
		  __entry->obj, __entry->handle,  __entry->fence_handle, __entry->waittime,  __entry->ret)
);
...
TRACE_EVENT(fence_releasebuf,
	...
	TP_printk("fence = %-3d, ret = %d",
		  __entry->fence_handle,  __entry->ret)
);
...
TRACE_EVENT(prime_handle_to_fd,
	...
	TP_printk("obj = %p, handle = %-3d, fd = %-3d, ret = %d",
		  __entry->obj, __entry->handle, __entry->fd,  __entry->ret)
);
...
TRACE_EVENT(prime_fd_to_handle,
	...
	TP_printk("fd = %-3d, handle = %-3d, ret = %d",
		  __entry->fd, __entry->handle,  __entry->ret)
);
...
TRACE_EVENT(prime_dmabuf_export,
	...
	TP_printk("paddr = %p, handle = %-3d, dmabuf = %p",
		  __entry->paddr, __entry->handle,	__entry->dmabuf)
);
...
TRACE_EVENT(prime_dmabuf_import,
	...
	TP_printk("dmabuf = %p, gemobj = %p",
		  __entry->dmabuf,	__entry->gem_obj)
);
...
TRACE_EVENT(prime_dmabuf_put,
	...
	TP_printk("paddr = %p, dmabuf = %p",
		  __entry->paddr, __entry->dmabuf)
);
...
//#define TRACE_INCLUDE_PATH /home/arshad/fresh/TBHSrc/hantrodriver/hantro_driver/
...
#define trace_client_add(file_attr, sliceidx, clientid, codec, profile, width, height)
#define trace_client_remove(file_attr, clientid)
#define trace_cma_alloc(sliceidx, region, paddr, handle, size)
#define trace_cma_free(sliceidx, paddr, handle)
#define trace_dec_reserve(sliceidx, core, waittime)
#define trace_dec_release(sliceidx, core)
#define trace_enc_reserve(sliceidx, core, waittime)
#define trace_enc_release(sliceidx, core)
#define trace_cache_reserve(sliceidx, waittime)
#define trace_cache_release(sliceidx)
